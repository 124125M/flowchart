...
import com.gmail.dailyefforts.flowchart.drawings.Drawing.Status;
...
public class DrawingPad extends View {
	...
	public DrawingPad(Context context, AttributeSet attrs) {
		...
		mBaseLinePaint.setStrokeWidth(10);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (mActiveDrawing.isActive()) {
				mResizeType = mActiveDrawing.getResizeType(x, y);
				if (mResizeType != ResizeType.NONE) {
					isResizing = true;
				} else if (mActiveDrawing.contains(x, y)) {
					isResizing = true;
					mActiveDrawing.setStatus(Status.MOVING);
				} else {
					isResizing = false;
					mActiveDrawing.setStatus(Status.DONE);
				}
			} else {
				for (Drawing drawing : mList) {
					mResizeType = drawing.getResizeType(x, y);
					if (mResizeType != ResizeType.NONE
							|| drawing.contains(x, y)) {
						mActiveDrawing = drawing;
	...
	private void fingerMove(float x, float y) {
		...
		mIsMoving = true;
		if (isResizing) {
			mActiveDrawing.move(x, y, mCanvas, mResizeType);
		}
	}
